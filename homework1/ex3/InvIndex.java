...
public class InvIndex extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		job.setCombinerClass(Combine_1.class);
	...
	public static class Combine_1 extends Reducer <Text, Text, Text, Text> {
		public void reduce(Text word, Iterable<Text> files,Context context) throws IOException, InterruptedException {
			...
			for(Text file : files) {
				//int value = mymap.get(file.toString());
				if (!mymap.containsKey(file.toString())) {
					mymap.put(file.toString(),1);
				} else {
					int incr = mymap.get(file.toString());
					incr++;
					mymap.put(file.toString(),incr);
				}
			...
			while( it.hasNext()) {
				...
				sb.append(pair.getKey()+"#"+String.valueOf(pair.getValue())+" ");
				it.remove();
			}
			context.write(new Text(word.toString()),new Text(sb.toString()));
	...
	public static class Reduce_1 extends Reducer <Text,Text,Text,Text> {
		public void reduce(Text word,Iterable<Text> files,Context context) throws IOException, InterruptedException {			StringBuilder sb = new StringBuilder();
		   for(Text file : files) {
				sb.append(file);
			}
		   context.write(new Text(++INDEX+" "+word.toString()),new Text(sb.toString())); 		
